...
@auth_bp.route('/addcontainer', methods=['POST'])
def addcontainer():
    container_id = request.json.get('containerId')
    session = request.json.get('session')
    ostype = request.json.get('ostype')
    user = request.json.get('userid')
    password = request.json.get('password')
    from auth.addcontainer import perform_addcontainer
    response = perform_addcontainer(container_id, ostype,user, password, session)
    return jsonify(response)
